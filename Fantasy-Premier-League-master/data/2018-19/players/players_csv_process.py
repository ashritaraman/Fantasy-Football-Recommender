...
df = pd.read_csv("weighted_nn_data_train.csv")
stuff = df.values.tolist()
for elem in stuff: 
    elem.remove(elem[0])
...
stuff1 = del_zero_rows()

df_fwd= pd.DataFrame(stuff1, columns = ['total_points_prev','assists','big_chances_created','clean_sheets','clearances_blocks_interceptions','dribbles','errors_leading_to_goal','ict_index','key_passes','penalties_missed','penalties_saved','total_points'])
df_fwd.to_csv("weighted_nn_data_train.csv")
